...
int main() {
    ...
    while(true) {
        ...
        da_make(&pipes, cmd_lines.len);
        pipes.len = cmd_lines.len;

        Pipe_Pair_Arr pipes_to_close = {0};
        int prev_in = -1;
        for(size_t i = 0; i < cmd_lines.len; ++i) {
            int pipe_pair[2] = {0};
            ...
            // For making cleanup easier
            Pipe_Pair pair = (Pipe_Pair){.in=pipe_pair[1], .out=pipe_pair[0]};
            da_append(&pipes_to_close, pair);
        }
        ...
        for(size_t i = 0; i < pipes_to_close.len; ++i) {
            close(pipes_to_close.items[i].in);
            close(pipes_to_close.items[i].out);
        }
        free(pipes_to_close.items);
    }
    free(cmds.items);
    free(procs.items);
